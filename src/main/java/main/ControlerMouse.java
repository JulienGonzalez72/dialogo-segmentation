...
public class ControlerMouse implements MouseListener {
		...
		if (view.player.isPhraseFinished() && e.getClickCount() < 2) {
			...
			if (handler.wordPause(offset) && handler.getPhraseIndex(offset) == view.player.getCurrentPhraseIndex()) {
				if ( FenetreParametre.modeSurlignage) {
					traitementClicJusteModeSurlignage(offset);
				} else {
				...
				if ( FenetreParametre.modeSurlignage) {
					traitementClicFauxModeSurlignage(offset);
				} else {
					traitementClicFaux(offset);
				}			
			}
		}
	}

	private void traitementClicFauxModeSurlignage(int offset) {
	...
	private void traitementClicJusteModeSurlignage(int offset) {
	...
	private void traitementClicFaux(int offset) {
		view.nbEssaisRestantPourLeSegmentCourant--;
		// si il reste un essai ou qu'une phrase est en train d'être corrigée
